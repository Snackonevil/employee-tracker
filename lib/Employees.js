...
// Import Roles object to help with queries
...
const Employees = {
    ...
    update: async (employeeId, firstName, lastName, newRole) => {
        // Get role ID by name
        const newRoleId = await Roles.getIdByName(newRole);
    ...
    delete: async (id, firstName, lastName) => {
        await db.query(
            `DELETE FROM employees WHERE id=${id} AND first_name ='${firstName}' AND last_name = '${lastName}'`
        );
        createLine(Red);
        console.log(
            Red,
            `${firstName.toUpperCase()} ${lastName.toUpperCase()}'s records were DELETED\n`
        );
        createLine(Red);
    },
};
